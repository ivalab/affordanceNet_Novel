...
OBJ_CLASSES = ('__background__', 'target', 'tool') # this is remapped from objectness to (1) object (2) tool
...
def write_pddl(path, list_obj_centroids):
    ...
    objects = '    (:objects arm '
    init =    '    (:init (free arm) '
    goal =    '    (:goal (and (scoops target tool))))'
    ...
    objects += ')\n'
    init += ')\n'
    with open(os.path.join(path, 'auto_problem_SCOOPABLE_binary.pddl'), 'w') as f:
