...
OBJ_CLASSES = ('__background__', 'bowl', 'cup', 'hammer', 'knife', 'ladle', 'mallet', 'mug', 'pot', 'saw', 'scissors','scoop','shears','shovel','spoon','tenderizer','trowel','turner','computer_mouse','fork','lipstick','pen','pizza_cutter','plate','remote_control','screwdriver','spoon','stapler','wrench') # this is mapped to exteneded classes of office table
AFF_CLASSES = ('', 'GRASPABLE', '', 'SCOOPABLE', 'CONTAINABLE', '', 'SUPPORTABLE', '', '', '')
...
def write_pddl(path, list_obj_centroids):
    ...
    objects = '    (:objects arm '
    init =    '    (:init (free arm) '
    goal =    '    (:goal (and (scooped bowl spoon))))'
    ...
    objects += ')\n'
    init += ')\n'
    with open(os.path.join(path, 'auto_problem_SCOOPBEANS.pddl'), 'w') as f:
