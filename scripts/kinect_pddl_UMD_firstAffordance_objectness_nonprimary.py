...
def visualize_mask_asus(im, rois_final, rois_class_score, rois_class_ind, masks, thresh, nonprimary = False):
    ...
    for i in xrange(0, num_boxes):
        ...
        if cfg.TEST.MASK_REG:
            ...
            else: # if the second affordance is chosen
                mask_ind = np.argmax(mask, axis=0)
                for i_ind in xrange(244):
                    for y_ind in xrange(244):
                        mask[mask_ind[i_ind, y_ind], i_ind, y_ind] = -1
                mask2 = np.argmax(mask, axis=0)
                for i_ind in xrange(244):
                    for y_ind in xrange(244):
                        if mask_ind[i_ind, y_ind] == 0:
                            mask2[i_ind, y_ind] = 0
                mask = mask2
            ...
            if not nonprimary:
                ...
                cv2.imshow('Mask_nonprimary' + str(i), color_curr_mask)   
            cv2.waitKey()
...
def run_affordance_net_asus(net, im):
    ...
    if cfg.TEST.MASK_REG:
        ...
        if args.sim and os.path.exists('/home/landan/data_shared/affordance/landan/support_nonprimary/im_detect2.npz'):
            container = np.load('/home/landan/data_shared/affordance/landan/support_nonprimary/im_detect2.npz')
            ...
            list_boxes, list_masks = visualize_mask_asus(im, rois_final, rois_class_score, rois_class_ind, masks, thresh=CONF_THRESHOLD)
            print len(list_boxes)

            rois_final, rois_class_score, rois_class_ind, masks, scores, boxes = container['arr_0'], container['arr_1'], container['arr_2'], container['arr_3'], container['arr_4'], container['arr_5']
            list_boxes2, list_masks2 = visualize_mask_asus(im, rois_final, rois_class_score, rois_class_ind, masks, thresh=CONF_THRESHOLD, nonprimary=True)    
            print len(list_boxes2)
    else:
        1
    timer.toc()
    ...
    return list_boxes + list_boxes2, list_masks + list_masks2
...
if __name__ == '__main__':
    ...
    if args.sim and os.path.exists('/home/landan/data_shared/affordance/landan/support_nonprimary/RGB.npy') and os.path.exists('/home/landan/data_shared/affordance/landan/support_nonprimary/depth.npy'):
        arr_rgb = np.load('/home/landan/data_shared/affordance/landan/support_nonprimary/RGB.npy')
        arr_depth = np.load('/home/landan/data_shared/affordance/landan/support_nonprimary/depth.npy')

        arr_rgb_show = np.concatenate((arr_rgb[:, :, 2:], arr_rgb[:, :, 1:2], arr_rgb[:, :, 0:1]), axis=2)
        import scipy.misc
        scipy.misc.imsave('screwdriver2knife.png', arr_rgb_show)
