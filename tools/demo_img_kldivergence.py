...
img_folder = cwd + '/img_UMD/TMP' #change the rank below
RANK = 1
...
def draw_reg_text(img, obj_info):
    ...
    draw_arrow(img, (xmin, ymin), (xmax, ymin), col_map[obj_id+3], 0, 5, 8, 0)
    draw_arrow(img, (xmax, ymin), (xmax, ymax), col_map[obj_id+3], 0, 5, 8, 0)
    draw_arrow(img, (xmax, ymax), (xmin, ymax), col_map[obj_id+3], 0, 5, 8, 0)
    draw_arrow(img, (xmin, ymax), (xmin, ymin), col_map[obj_id+3], 0, 5, 8, 0)
...
def visualize_mask(im, rois_final, rois_class_score, rois_class_ind, masks, ori_height, ori_width, im_name, thresh, affordanceRank):
    ...
    for i in xrange(0, num_boxes):
        ...
        if cfg.TEST.MASK_REG:
            ...
            elif affordanceRank == 2:
              # if the second affordance is chosen
            ...
            elif affordanceRank == 3:
              # if the third affordance is chosen
              mask_ind = np.argmax(mask, axis=0)
              for i_ind in xrange(244):
                  for y_ind in xrange(244):
                      mask[mask_ind[i_ind, y_ind], i_ind, y_ind] = -1
              mask_ind2 = np.argmax(mask, axis=0)
              for i_ind in xrange(244):
                  for y_ind in xrange(244):
                      mask[mask_ind2[i_ind, y_ind], i_ind, y_ind] = -1

              mask3 = np.argmax(mask, axis=0)
              for i_ind in xrange(244):
                  for y_ind in xrange(244):
                      if mask_ind[i_ind, y_ind] == 0 or mask_ind2[i_ind, y_ind] == 0 :
                          mask3[i_ind, y_ind] = 0
              mask = mask3
...
def run_affordance_net(net, image_name):
    ...
    visualize_mask(im, rois_final, rois_class_score, rois_class_ind, masks, ori_height, ori_width, im_name, thresh=CONF_THRESHOLD, affordanceRank=RANK)
...
if __name__ == '__main__':
    ...
    caffemodel = root_path + '/pretrained/vgg16_faster_rcnn_iter_192000_category_kl.caffemodel'   
